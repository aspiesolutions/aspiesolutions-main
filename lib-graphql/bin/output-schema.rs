...
fn main() -> std::io::Result<()> {
    ...
    let schema = lib_graphql::Schema::new(
        ...
        lib_graphql::EmptySubscription::<lib_graphql::Context>::default(),
    );
    ...
    let schema_output_name =
        std::env::var("SCHEMA_OUTPUT_NAME").unwrap_or_else(|_| String::from("schema.graphql"));
    let path = PathBuf::from(schema_output_directory);
    std::fs::create_dir_all(&path)?;
    let mut schema_file = File::create(path.join(schema_output_name))?;
    let _ = schema_file.write(schema_string.as_bytes())?;
    schema_file.flush()?;
