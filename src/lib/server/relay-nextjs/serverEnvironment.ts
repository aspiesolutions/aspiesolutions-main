...
export function createServerNetwork(token: string) {
  return Network.create(async (params, variables) => {
    // relay crashes the program if no data is returned with errors.
    // convert the "errors" into data.errors to allow the program
    // to continue handling the error
    let relay_response = { data: null, errors: null } as any;
    try {
      ...
      relay_response.data = response.data?.data;
      relay_response.errors = response.data?.errors;
    } catch (e) {
      console.error(e);
